...
CREATE OR REPLACE PROCEDURE test.test_get_items_by_user_pages()
AS
$$
DECLARE
    seller_id      UUID;
...
BEGIN
    ...
    SELECT total_pages
    INTO items_returned
    FROM get_items_by_user_pages(seller_id);
    ...
    SELECT total_pages
    INTO items_returned
    FROM get_items_by_user_pages(seller_id, FALSE, 10);
    ...
    SELECT total_pages
    INTO items_returned
    FROM get_items_by_user_pages(seller_id, TRUE, 6);
